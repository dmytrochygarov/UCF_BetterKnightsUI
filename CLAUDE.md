...
- **Release**: bump `version` in **all three** files in [manifests/](manifests/) — they are in lockstep (1.8) and nothing enforces that — then zip the *contents* of `build/<browser>/` so `manifest.json` lands at the zip root. On Windows, PowerShell's `Compress-Archive` writes `\` separators into the archive and the stores choke on it; build the zip with .NET (`[IO.Compression.ZipFile]::Open` + `CreateEntryFromFile` with forward-slash entry names) instead.

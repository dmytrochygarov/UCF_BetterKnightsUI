...
function isMyClassScheduleListView(root) {
  ...
  // List view shows CLASS_MTG_VW meeting grids. Weekly Schedule and class
  // search use different markup.
...
function extractEnrolledMeetings(root) {
  ...
  courses.forEach((courseEl) => {
    ...
    const identity = parseCourseTitle(
      titleEl ? textContent(titleEl) : ""
    );
    if (!identity) return;
...
function parseMeetingRow(row, identity, carry) {
  ...
  const modeRaw =
    fieldText(row, "INSTRUCT_MODE_DESCR") ||
    fieldText(row, "SSR_INSTR_MODE");
...
root.isMyClassScheduleListView = isMyClassScheduleListView;
root.extractEnrolledMeetings = extractEnrolledMeetings;

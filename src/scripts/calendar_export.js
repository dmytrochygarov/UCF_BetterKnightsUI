...
(function () {
  ...
  // RFC 5545 requires a VTIMEZONE for every referenced TZID; the post-2007
  // US Eastern rules cover every date this extension will ever emit.
  ...
  function buildCalendarExport(meetings) {
    ...
    const skips = [];
    // Fresh DTSTAMP per build so calendar apps treat a re-import with the
    // same UID as an update, not a stale copy.
    const dtStamp = formatUtcStamp(new Date());
    const uidCounts = new Map();

    for (const meeting of list) {
      const outcome = buildVEvent(meeting, dtStamp, uidCounts);
      if (outcome.vevent) {
  ...
  function buildVEvent(meeting, dtStamp, uidCounts) {
    ...
    const uid = stableUid(meeting, days, uidCounts);
  ...
  /**
   * Durable Meeting identity only (class number + type + day pattern) so a
   * re-export after times change keeps the UID and updates prior events.
   * ponytail: suffix disambiguates same-pattern duplicates by row order; if
   * PeopleSoft reorders such rows between exports, those UIDs swap.
   */
  function stableUid(meeting, byDays, uidCounts) {
    const key = sanitizeUid(
      [
    ...
    const n = (uidCounts.get(key) || 0) + 1;
    uidCounts.set(key, n);
    return (n > 1 ? `${key}-${n}` : key) + "@betterknightsui";
  }
